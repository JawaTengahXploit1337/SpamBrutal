...
$               $$$$$$  $$$$$$
                  $$$$  $$$$
                    $$  $$
                     $  $
# %sAuthor By %s./W4D3R 1337%s
# %sGithub %sgithub.com/JawaTengahXploit1337%s
# %sWebsite %shttp://www.jawatengahxploit.tech%s
...
def input():
  ...
  if len(nom) < 5:
    print "%s[%s!%s] %sMasukkan Nomor Target Dengan Benar Cuk !!"%(pu,me,pu,me)
    input()
...
def klikwa():
 ...
 if json.loads(tes.text)["message"] == 'OTP Sent':
  print "     %s[%s%s%s] [%s Sent %s] %sSuccess, spam %s from %s%s %ssended %s>> %sMau yang unlimited? %shttp://www.jawatengahxploit.tech"%(pu,ku,"10",pu,hi,pu,pu,"wa",ku,"klikwa",hi,qu,pu,ku)
 else:
...
if __name__ == '__main__':
 ...
 except requests.exceptions.ConnectionError: exit("%s[%s!%s] %sConnection Error Cuk..."%(pu,me,pu,me))
